...
- [ ] Gate 失败 → 自动二次 `codeact:run` **live 验证**（须刻意 FAIL 场景）
- [ ] T7 live **批量** N≥3 成功率（方差口径）
- [ ] 1 条 DeepSeek live 冒烟入 CI（密钥 gated workflow）
- [ ] `llm_usage` token 统计修复（`MetricsSnapshot` 字段）
...
**已知小问题**：`llm_usage` 事件报 `MetricsSnapshot` 无 `prompt_tokens` 属性（不影响 Gate pass；Phase 1 待修）。
...
1. 修 `llm_usage` / `MetricsSnapshot` token 字段  
2. Gate 失败回流 **live** 验证（构造 FAIL → fix_prompt → 二次 codeact）  
3. T7 live batch N≥3 + T4 live hybrid PoC  
4. `@stagent/core` 内 `tool: 'codeact'` + `STAGENT_IMPL_ENGINE=legacy`  
5. Electron `codeact-bridge.ts` 三步 UI  
6. `examples/bundles/t4-nanhua/` 补真实 fixture CSV  
