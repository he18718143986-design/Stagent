...
| **5** | **瘦身 Stagent + 内嵌 CodeAct + 重 Gate（商业化 hybrid）** | **P0** | 方案 A+B | `cursor/deliver-t4-mainland-170f` | ✅ **T4 一键交付 + live strict pass**（2026-06-18，`deliver:t4` attempt 3/3 含 Gate 回流）；T7 live ✅；余：T4 batch、引擎 `tool:codeact` | PR 待开 |

**#5 hybrid-codeact — T4 大陆一键交付（2026-06-18）**：
- **命令**：`npm run deliver:t4 -- --workspace /tmp/deliver-t4-live-poc --force`
- **结果**：fixture 种子 → CodeAct ×3（Gate 回流 2 次）→ **strict pass attempt 3/3**；pytest 3 files；G-signals-nonzero ✓
- **Runbook**：`docs/comercial/交付-runbook-hybrid.md`
- **npm**：`deliver:t4` / `deliver:t4:batch`；`feedback:live:t4` 默认 `--runner hybrid`
- **T7 live**（已完成）：`npm run hybrid:t7` attempt 1/1 strict pass（§10.A）
- **残留**：T4 live batch N≥3；引擎 `tool:codeact`；Electron bridge

**#5 hybrid-codeact 后（2026-06-18）— T7 live PoC**：
- **命令**：`npm run hybrid:t7 -- --workspace /tmp/hybrid-live-t7-poc --force`（DeepSeek，`LLM_BASE_URL=https://api.deepseek.com/v1`）
- **结果**：export → CodeAct（~2min）→ `gate:strict` **pass**，attempt **1/1**，pytest **13 passed**，`output.json` 非全零
- **证据**：`docs/plans/slim-stagent-codeact-integration.md` §10.A；PR #31
- **残留**：`llm_usage` token 字段兼容；Gate 回流 live 未测；T4 live batch 未跑
