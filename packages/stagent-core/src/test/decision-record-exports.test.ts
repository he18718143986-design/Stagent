...
test('resolveModuleExports：slice 契约全是占位/噪声（仅 [DictReader]）→ 落到 global 兜底（1e/3b run#2）', () => {
  const slice = { version: 1 as const, files: [], modules: [{ name: 'pipeline', exports: ['DictReader'] }] };
  const global = { version: 1 as const, files: [], modules: [{ name: 'pipeline', exports: ['import_tasks_from_csv', 'summarize'] }] };
  assert.deepEqual([...(resolveModuleExports('pipeline', slice, global) ?? [])].sort(), [
    'import_tasks_from_csv',
    'summarize',
  ]);
});

test('pruneExportNoise 剔除 csv 读写器成员 DictReader/DictWriter（占位，非模块 API）', () => {
  assert.deepEqual(pruneExportNoise(['DictReader', 'import_tasks_from_csv', 'DictWriter', 'summarize']), [
    'import_tasks_from_csv',
    'summarize',
  ]);
});
